...
- **Hybrid search over the corpus** — fast, accurate retrieval across 13.5M chunks of law. The user doesn't call this. The intelligence layer does, on their behalf, with their situation as context.
...
**Canonical substrate underneath the world model.** The structured understanding above sits on a normalized data substrate that every loader (cases, statutes, regulations, municipal codes) hits through one shared contract — `documents` ↔ `chunks` joined to a `jurisdictions` reference table, with `source_provenance` recording which ingestion path produced which version. The substrate is the prerequisite for cross-source treatment work and for the Path B state-statute corpus expansion; without it, jurisdiction filtering silently fails on non-cases and per-source-flavored fields leak into synthesis. Substrate details are an implementation concern, not architecture — the data-model spec lives in [`openprecedent-pipeline/CANONICAL_DATA_MODEL_SPEC.md`](https://github.com/thecloudgeek/openprecedent-pipeline/blob/main/CANONICAL_DATA_MODEL_SPEC.md) and the backend migration mechanics in [`canonical-document-schema.md`](https://github.com/thecloudgeek/openprecedent-backend/blob/main/docs/specs/canonical-document-schema.md). The architectural rule is just that the world model reads from the canonical shape, never from loader-specific fields.
